...
p.add('-d', '--debug', required=False, help='Turn on debug output', default=False)
p.add('-v', '--verbose', required=False, help='Print program flow section headers.', default=True)
...
try:
    ...
    while True:
                ...
                for item in bitlinq.messages:
                    ...
                    if message2 in sent_messages:
                        ...
                        if bitlinq.gateway_ID[0] == TTN_GATEWAY_ID_LS1:
                            if options.debug:
                                print(str(datetime.now()) + ": Message received from Lacuna Space LS1 payload on-board M6P satellite!!!")
        ...
        if options.verbose:
            print(colored('################ bitlinq_peerdid verify received connection invitation and create invitation response ################' + '\n', 'yellow'))
        incoming_base64_payload = bitlinq.repack_messages_list(payload)
        if options.debug:
        ...
        if result:
            if options.debug:
                ...
                print(leogeo_did1.verify(diddocV, vk, bytearray.fromhex(sig)))
        ...
        if options.verbose:
            print(colored('################ bitlinq_peerdid verify connection response ################' + '\n','cyan'))
        ...
        if options.verbose:
            print(colored('################ bitlinq_peerdid establish messaging ################' + '\n', 'magenta'))
        ...
        if options.debug:
            ...
            print('The verification of the didcomm message result: {}'.format(result))
